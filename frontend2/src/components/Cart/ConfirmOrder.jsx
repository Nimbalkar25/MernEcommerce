...
const ConfirmOrder = () => {
  ...
  const proceedToPayment = async () => {
    try {
      // Optional: backend-side validation (or optimistic validation)
      for (const item of cartItems) {
        const { data } = await axios.get(`/api/v1/product/${item.product}`);
        const latestStock = data.product.Stock;

        console.log(
          `Checking item: ${item.name}, Quantity: ${item.quantity}, Latest Stock: ${latestStock}`
        );
        if (item.quantity > latestStock) {
          alert(`${item.name} is out of stock. You’ll be redirected to Cart.`);
